	...
	Convey("Testing cache operation", t, func() {
		SimulateInNewEnvironment(security.SuperUserID, func(env Environment) {
			...
			Convey("Loading a RecordSet should populate the cache", func() {
				...
				janeCacheRef := userJane.getFirstCacheRef()
				So(env.cache.data, ShouldContainKey, janeCacheRef)
				So(env.cache.getData(janeCacheRef), ShouldContainKey, "id")
				So(env.cache.getData(janeCacheRef)["id"], ShouldEqual, userJane.ids[0])
				So(env.cache.getData(janeCacheRef), ShouldContainKey, "name")
				So(env.cache.getData(janeCacheRef)["name"], ShouldEqual, "Jane A. Smith")
				So(env.cache.getData(janeCacheRef), ShouldContainKey, "email")
				So(env.cache.getData(janeCacheRef)["email"], ShouldEqual, "jane.smith@example.com")
			...
			Convey("Testing O2M fields in cache", func() {
				...
				janeCacheRef := userJane.getFirstCacheRef()
				So(env.cache.data, ShouldContainKey, janeCacheRef)
				So(env.cache.getData(janeCacheRef), ShouldContainKey, "posts_ids")
				So(env.cache.getData(janeCacheRef)["posts_ids"], ShouldEqual, true)
			...
			Convey("Check that computed fields are stored and read in cache", func() {
				...
				So(env.cache.getData(janeCacheRef), ShouldContainKey, "id")
				So(env.cache.getData(janeCacheRef), ShouldContainKey, "name")
				So(env.cache.getData(janeCacheRef), ShouldNotContainKey, "decorated_name")
				decoratedName := userJane.Get("DecoratedName")
				So(env.cache.getData(janeCacheRef), ShouldContainKey, "decorated_name")
				So(env.cache.getData(janeCacheRef)["decorated_name"], ShouldEqual, decoratedName)
			})
		})
	})
}
